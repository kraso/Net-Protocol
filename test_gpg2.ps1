...
if ($LASTEXITCODE -eq 0) {
  "FRASE OK -> subiendo al secreto"
  $p | gh secret set GPG_PASSPHRASE -R kraso/redes-knowledge
  if ($LASTEXITCODE -eq 0) { "SECRETO ACTUALIZADO" } else { "FALLO al subir el secreto (codigo $LASTEXITCODE)" }
} else { "FRASE INCORRECTA - revisala y reintenta" }
