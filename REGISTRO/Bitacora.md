...
## 28-08-2026 — Incidencia de cuota de artefactos en CI + rename del repositorio

**Incidencia (v1.0.3):** el pipeline falló en `upload-artifact` con **`Artifact storage quota has been hit`** en build macOS (1.er intento) y Linux (2.º). Los builds self-contained suben ~176 MB por run (app-* ×3) más los instaladores (setup-* ×3), y con la cuota de **500 MB de artefactos de los repos privados** y la retención por defecto de 90 días, los runs acumulados de la fase de desarrollo la agotaron. **Causas raíz:** (1) artefactos de runs antiguos sin expirar; (2) sin `retention-days` en los uploads; (3) el **uso de la cuota se recalcula cada 6–12 h**, así que relanzar inmediatamente tras borrar seguía fallando hasta que Google… *GitHub* actualiza la marca.

**Arreglos:** se eliminaron **271 artefactos** vía API (`DELETE /repos/{owner}/actions/artifacts/{id}`; el bucle inicial con `-o $null` no borraba y enmascaraba errores — repetido iterando y comprobando `$LASTEXITCODE`); cada `upload-artifact` del workflow ahora lleva **`retention-days: 2`** (los instaladores son los únicos artefactos con valor a largo plazo; el release job los adjunta al release). Lección anotada en el propio workflow.

**Rename del repositorio (decisión del responsable):** `kraso/redes-knowledge` → **`kraso/Net-Protocol`** (privado; `gh repo rename` + `git remote set-url origin`). Los secretos GPG (`GPG_PRIVATE_KEY`/`GPG_KEY_ID`/`GPG_PASSPHRASE`) sobreviven al rename. Los enlaces antiguos de GitHub redirigen automáticamente.

**Desbloqueo de v1.0.3 (decisión del responsable):** para levantar la cuota de artefactos sin esperar el recálculo de 6–12 h, el repositorio pasó a **público** (`gh repo edit --visibility public --accept-visibility-change-consequences`) el 28-08-2026 — el almacenamiento de artefactos de repos públicos es gratuito e ilimitado. **v1.0.3 publicado con CI ✅** (`NetProtocol-Setup-1.0.3.exe`, `.deb` + `.asc` verificado localmente: *"Firma correcta de MARCOS CALABRES IBAÑEZ [absoluta]"*, `.rpm`, `.dmg`, `NetProtocol-gpg-pubkey.asc`) con el **grafo navegable incluido** (la corrección que motivó la versión). La decisión de visibilidad pública quedaba marcada como pendiente en el backlog y se resolvió con este motivo.
## 28-08-2026 — Instalador robusto: AppMutex, cierre automático y limpieza de huérfanos (v1.0.4)

**Consulta del responsable:** *"¿instalar una versión nueva requiere desinstalar la anterior o se superponen con seguridad?"* — Revisado el `.iss`: Inno deriva el `AppId` de `AppName`+`AppPublisher`+`DefaultDirName` (clave de desinstalación `Net Protocol_is1` en el registro local), estable entre versiones → **actualiza en el mismo directorio sin desinstalar**. Los datos de usuario (DB, capturas) viven en `%LOCALAPPDATA%\NetProtocol`, fuera de `{app}`: ni actualizar ni desinstalar los toca. Los límites del montaje: sin `AppMutex` (sobreescribía con la app abierta → ficheros bloqueados) y sin `[InstallDelete]` (posibles huérfanos de versiones previas).

**Mejoras implementadas y validadas (build 0/0 · .iss compilado con ISCC 6 ✅):**
- `Program.cs`: la app crea el **mutex `NetProtocolMutex`** mientras está abierta (verificado: visible con la app abierta, liberado al cerrar; no afecta al modo CLI `--l004`).
- `.iss`: **`AppMutex=NetProtocolMutex`** + **`CloseApplications=force`** + **`CloseApplicationsFilter=NetProtocol.exe`** (el instalador detecta la app abierta y la cierra sola antes de sobreescribir) y **`[InstallDelete]` `Type: filesandordirs; Name: "{app}\*"`** (limpia huérfanos antes de copiar; seguro porque los datos viven en LOCALAPPDATA). **Sin `AppId` explícito**: fijar un GUID propio rompería el reconocimiento de las instalaciones ya hechas (clave `Net Protocol_is1`), así que se deja el autogenerado estable.
...
**Ajuste posterior:** texto del tooltip de **Abrir captura** → "Abre una ventana del explorador para que selecciones el archivo .cap de captura de paquetes con PCAP/PCAPNG y luego muestra sus detalles en pantalla." Y el tooltip de **Muestra de prueba** ahora **incluye la carpeta de capturas real del sistema** (`%LOCALAPPDATA%\NetProtocol\capturas`, calculada en tiempo de ejecución con `ToolTip.SetTip`, la misma ruta que usa `GenerarMuestra`) para que el usuario localice las capturas generadas. Ajuste de ancho: `MaxWidth 640 → 1100` px con `NoWrap` + `CharacterEllipsis` — la frase + `C:\` + ruta salen enteras en una sola línea; si una ruta extrema no cabe, el recorte cae SIEMPRE al final (el inicio `C:\…` queda intacto).

**Publicación:** **v1.0.7** con CI verde (`<Version>` a 1.0.7; el pipeline construye los instaladores con el tag).
## 28-08-2026 — Tooltip de Muestra de prueba: causa raíz (versión instalada) y fix con SetTipTemplate
...
## 29-08-2026 — Publicación v1.0.9 (CI verde, run 33198582037)

**Release publicado:** https://github.com/kraso/Net-Protocol/releases/tag/v1.0.9 con 6 assets: `NetProtocol-Setup-1.0.9.exe`, `NetProtocol-1.0.9-amd64.deb` + `.asc`, `NetProtocol-1.0.9-x86_64.rpm`, `NetProtocol-1.0.9-macos.dmg`, `NetProtocol-gpg-pubkey.asc` (firma GPG verificada en CI: `rpm --checksig` y `gpg --verify`). El RPM **ya incluye** entrada de menú, icono y comando `netprotocol`; todos los paquetes llevan la URL ascendente `https://github.com/kraso/Net-Protocol` (derivada del repo) y la app **arranca en tema oscuro**. Instalación en openSUSE: `sudo rpm --import NetProtocol-gpg-pubkey.asc && sudo zypper install NetProtocol-1.0.9-x86_64.rpm` (o `--gpg-auto-import-keys`) — la app aparece en el menú con su icono y se lanza con `netprotocol`.

## 29-08-2026 — v1.0.9 (rpm y deb): binario sin bit de ejecución → menú oculto y chmod manual (fix + re-corte de la release)

**Síntoma del responsable (openSUSE):** la v1.0.9 del RPM seguía sin crear la entrada de menú, y para lanzar el programa había que darle permisos de ejecución a mano a `/usr/bin/netprotocol` ("cosa que no es habitual").

**Diagnóstico (inspección del RPM y el DEB publicados, extraídos con 7-Zip):** el enlace `/usr/bin/netprotocol` es correcto (symlink 777 → `/usr/lib/netprotocol/NetProtocol`, tamaño 32 = ruta + NUL), el `.desktop` es correcto, pero **el ELF `NetProtocol` (y `createdump`) está en modo `0644` (sin +x) en AMBOS paquetes**. Causa: `actions/download-artifact@v4` **pierde el bit de ejecución** al descargar (problema conocido del action); el `cp -a` copia fielmente ese mode sin +x, y `chmod -R a+rX` no remedia (la `X` mayúscula solo añade x a lo que ya tiene algún x). Consecuencias: el lanzamiento por `netprotocol` falla hasta el `chmod +x` manual, y **KDE oculta la entrada de menú** porque valida la ejecutabilidad del `Exec` (por eso tampoco aparecía). En la v1.0.8 el usuario ya sufrió la misma pérdida (la arregló a mano) — el `.desktop` nuevo expuso el problema.

**Fix (CI, `package-linux`):** fijar **`0755` explícito** en los apphosts ELF (`NetProtocol`, `createdump`) al empaquetar — en el `debroot` y en el `%install` del spec — sin depender del mode del artefacto; y `%post` en el RPM con refresco best-effort del menú (`update-desktop-database` + `kbuildsycoca6 --noincremental`, ambos con `|| true`). **Release v1.0.9 borrada y re-cortada** desde el commit corregido (precedente v1.0.8): `gh release delete --yes --cleanup-tag` + re-tag + push; tras CI verde se verificó el nuevo RPM extrayéndolo: `NetProtocol → -rwxr-xr-x`.

**Verificación del fix (post re-corte):** el `.rpm` de la nueva v1.0.9 inspeccionado localmente muestra `Mode = -rwxr-xr-x` en `usr/lib/netprotocol/NetProtocol` y `usr/lib/netprotocol/createdump`; el symlink `usr/bin/netprotocol` → `/usr/lib/netprotocol/NetProtocol`; `.desktop` y `netprotocol.png` presentes. Con la actualización en openSUSE, la entrada de menú aparece (o con `kbuildsycoca6 --noincremental`) y `netprotocol` lanza la app sin tocar permisos.

## 29-08-2026 — Estudio de colorido y estética de la interfaz (documento de diseño)

**Pedido del responsable:** analizar cómo mejorar el colorido de la interfaz para que el **gradiente de la barra de título** y los **bordes de los recuadros seleccionados** no rompan la estética; revisar detalles y mejoras implementables.

**Entregado** `FASE-II-DISENO/F2I-UX-Colorido-y-Estetica.md` con: inventario de los ~25 colores actuales (la app tiene identidad slate profesional `#0F172A`/`#334155` + acento de enlace `#6CBAFF`, pero hardcodeados y dispersos); diagnóstico — **(P-A1)** el gradiente negro→**blanco** de la barra deja una franja blanca en el extremo derecho que **deja los botones de ventana blancos sobre blanco** (≈1,1:1, ilegibles) y choca con el tema oscuro por defecto; **(P-B1)** la selección la pinta el acento cian de fábrica del tema Fluent (`#2882E6`/`#4CC2FF`), ajeno a la paleta; **(P-C1)** divisor `#334155` fijo y diagramas SIEMPRE claros ("islas blancas" en oscuro). Propuesta: sistema de **16 tokens** (claro/oscuro vía `ThemeDictionaries`), gradiente "cristal pizarra" (termina en slate-600 translúcido, nunca blanco → botones a ≈7,5:1), estilos de selección con `tSeleccionFondo`/`tSeleccionBorde`, variante oscura de `DiagramView` y 12 mejoras P0/P1/P2. Contraste WCAG verificado (todo AA; el gradiente nuevo MEJORA el peor caso actual).

**Implementación (paquete P0+P1 completo, aprobado por el responsable):** tokens por tema en `App.axaml`; selección/foco `ListBox`/`ComboBox`/`TextBox` con la paleta de la app (con `Foreground=tTexto` en :selected — el tema Fluent pone texto blanco); divisores y sidebar a tokens; botones de ventana (glifo/hover/cerrar→`tPeligro`); popup resuelto desde tokens con respaldo; `AlternarTema` repinta los diagramas; `DiagramView` con paleta por tema (fondo pizarra en oscuro, texto `#E2E8F0`, cajas slate; el `#0f172a` del layout se reinterpreta al color del tema; sin `Application.Current` → paleta clara, determinismo intacto). **Build 0 errores · 78/78 tests.**

## 30-08-2026 — Publicación v1.0.10 (CI verde, run 33293059933) — UX colorida

**Release publicado:** https://github.com/kraso/Net-Protocol/releases/tag/v1.0.10 con 6 assets (`NetProtocol-Setup-1.0.10.exe`, `NetProtocol-1.0.10-amd64.deb` + `.asc`, `NetProtocol-1.0.10-x86_64.rpm`, `NetProtocol-1.0.10-macos.dmg`, `NetProtocol-gpg-pubkey.asc`). Contenido: **paleta por tema y tokens** (gradiente "cristal pizarra" — los botones de ventana dejan de ser blancos sobre blanco; selección/foco con `tSeleccionFondo`/`tAcento`; divisores dinámicos; diagramas adaptativos al modo oscuro). Commit `18f1fc7` (UX) + `92ab393` (bump 1.0.10). Pendiente: verificación visual del responsable (posible afinado del overlay de selección Fluent).