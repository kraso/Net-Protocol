...
**Implementación (paquete P0+P1 completo, aprobado por el responsable):** tokens por tema en `App.axaml`; selección/foco `ListBox`/`ComboBox`/`TextBox` con la paleta de la app (con `Foreground=tTexto` en :selected — el tema Fluent pone texto blanco); divisores y sidebar a tokens; botones de ventana (glifo/hover/cerrar→`tPeligro`); popup resuelto desde tokens con respaldo; `AlternarTema` repinta los diagramas; `DiagramView` con paleta por tema (fondo pizarra en oscuro, texto `#E2E8F0`, cajas slate; el `#0f172a` del layout se reinterpreta al color del tema; sin `Application.Current` → paleta clara, determinismo intacto). **Build 0 errores · 78/78 tests.** Pendiente: verificación visual del responsable (posible afinado del overlay de selección Fluent) y commit.